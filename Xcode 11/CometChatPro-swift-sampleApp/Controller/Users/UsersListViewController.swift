...
extension UsersListViewController : UITableViewDelegate, UITableViewDataSource {
   ...
   //numberOfRowsInSection:
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
       let cell = oneOneOneTableView.dequeueReusableCell(withIdentifier: "UsersTableViewCell") as! UsersTableViewCell
        
               if !usersArray.isEmpty {
                   
                   if isFiltering() {
                       buddyData = self.filteredUsersArray.item(at: indexPath.row)
                   } else {
                       buddyData = self.usersArray.item(at: indexPath.row)
                   }
                   ...
                   cell.getUnreadCountForAllUsers(UID: buddyData?.uid ?? "0") { (count, error) in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let selectedCell:UsersTableViewCell = tableView.cellForRow(at: indexPath) as! UsersTableViewCell
...
extension UsersListViewController : UISearchBarDelegate, UISearchResultsUpdating {
...
extension UsersListViewController : CometChatUserDelegate {
...
extension UsersListViewController : CometChatMessageDelegate {
    
    func onTextMessageReceived(textMessage: TextMessage) {
        if let row = self.usersArray.firstIndex(where: {$0.uid == textMessage.senderUid && textMessage.receiverType == .user}) {
            let indexPath = IndexPath(row: row, section: 0)
            DispatchQueue.main.async {
                let cell = self.oneOneOneTableView.cellForRow(at: indexPath) as! UsersTableViewCell
