...
extension CometChatSnackBoard {
    ...
    public static func showErrorMessage(for error: CometChatException) {
        ...
        if error.errorCode == "ERROR_INTERNET_UNAVAILABLE" {
            CometChatSnackBoard.display(message:  "ERROR_INTERNET_UNAVAILABLE" , mode: .error, duration: .short)
        }else{
            CometChatSnackBoard.display(message:  "SOMETHING_WENT_WRONG_ERROR" , mode: .error, duration: .short)
        }
    }
}
