...
extension Notification.Name {
    ...
    /// By checking the array, you could know the hash codes of files are removed.
     static let CometChatKingfisherDidCleanDiskCache =
...
/// Key for array of cleaned hashes in `userInfo` of `CometChatKingfisherDidCleanDiskCacheNotification`.
 let CometChatKingfisherDiskCacheCleanedHashKey = "com.onevcat.CometChatKingfisher.cleanedHash"
...
/// Represents the caching operation result.
 struct CacheStoreResult {
    ...
    /// you can get it from `.failure` case of this `diskCacheResult`.
     let diskCacheResult: Result<(), CometChatKingfisherError>
}

extension CFCrossPlatformImage: CacheCostCalculable {
    /// Cost of an image
     var cacheCost: Int { return cf.cost }
}
...
/// the memory and disk storage config.
 class ImageCache {
    ...
    /// for any of your customize cache.
     static let default1 = ImageCache(name: "default")
    ...
    /// the storage `config` and its properties.
     let memoryStorage: MemoryStorage.Backend<CFCrossPlatformImage>
   ...
   convenience init(name: String) {
    ...
    // MARK: Storing Images
     ...
     func store(_ image: CFCrossPlatformImage,
                    original: Data? = nil,
                    forKey key: String,
    ...
    ///            `.none` means the image is not in cache or it is already expired.
     func imageCachedType(
        forKey key: String,
        processorIdentifier identifier: String = DefaultImageProcessor.default1.identifier) -> CacheType
    {
        let computedKey = key.computedKey(with: identifier)
    ...
    /// use `imageCachedType(forKey:processorIdentifier:)` instead.
     func isCached(
        forKey key: String,
        processorIdentifier identifier: String = DefaultImageProcessor.default1.identifier) -> Bool
    {
    ...
    /// if you need.
     func hash(
        forKey key: String,
        processorIdentifier identifier: String = DefaultImageProcessor.default1.identifier) -> String
    {
        let computedKey = key.computedKey(with: identifier)
    ...
    /// - Parameter handler: Called with the size calculating finishes. This closure is invoked from the main queue.
     func calculateDiskStorageSize(completion handler: @escaping ((Result<UInt, CometChatKingfisherError>) -> Void)) {
        ioQueue.async {
            do {
