...
/// CometChatKingfisherOptionsInfo is a typealias for [CometChatKingfisherOptionsInfoItem].
/// You can use the enum of option item with value to control some behaviors of CometChatKingfisher.
 typealias CometChatKingfisherOptionsInfo = [CometChatKingfisherOptionsInfoItem]

extension Array where Element == CometChatKingfisherOptionsInfoItem {
    static let empty: CometChatKingfisherOptionsInfo = []
}

/// Represents the available option items could be used in `CometChatKingfisherOptionsInfo`.
 enum CometChatKingfisherOptionsInfoItem {
    
    /// CometChatKingfisher will use the associated `ImageCache` object when handling related operations,
    ...
    ///
    /// When using CometChatKingfisherManager to download and store an image, if `cacheOriginalImage` is
    ...
    /// CometChatKingfisher will try to search the original image to check whether it is already there. If found,
    ...
    /// CometChatKingfisher will use the associated `ImageDownloader` object to download the requested images.
    case downloader(ImageDownloader)

    /// Member for animation transition when using `UIImageView`. CometChatKingfisher will use the `ImageTransition` of
    ...
    /// If set, CometChatKingfisher will ignore the cache and try to fire a download task for the resource.
    case forceRefresh

    /// If set, CometChatKingfisher will try to retrieve the image from memory cache first. If the image is not in memory
    ...
    /// If set, CometChatKingfisher will only cache the value in memory but not in disk.
    case cacheMemoryOnly
    
    /// If set, CometChatKingfisher will wait for caching operation to be completed before calling the completion block.
    case waitForCache
    
    /// If set, CometChatKingfisher will only try to retrieve the image from cache, but not from network. If the image is
    ...
    /// retrieving images from cache. If not set, CometChatKingfisher will use main queue for callbacks.
    ...
    /// cache. If not set, CometChatKingfisher will use `.mainCurrentOrAsync` for callbacks.
    ///
    /// - Note:
    ...
    /// with 2x or 3x retina images. Otherwise, CometChatKingfisher will convert the data to image object at `scale` 1.0.
    ...
    /// you should choose the image view class to control the GIF data loading. There are two classes in CometChatKingfisher
    ...
    /// CometChatKingfisherManager or any of the view extension methods), the converted image will also be sent to cache as well.
    ...
    /// If set, CometChatKingfisher will only load the first frame from an animated image file as a single image.
    ...
    /// If set and an `ImageProcessor` is used, CometChatKingfisher will try to cache both the final result and original
    /// image. CometChatKingfisher will have a chance to use the original image when another processor is applied to the same
    ...
    /// If set and a downloading error occurred CometChatKingfisher will set provided image (or empty)
    ...
    /// in disk cache, CometChatKingfisher will not try to load it to memory.
    ...
    /// if you are using CometChatKingfisher's extension methods to set an image), with a tradeoff of loading performance.
    ...
    /// Decides on which queue the image processing should happen. By default, CometChatKingfisher uses a pre-defined serial
    ...
    /// Enable progressive image loading, CometChatKingfisher will use the `ImageProgressive` of
...
// Improve performance by parsing the input `CometChatKingfisherOptionsInfo` (self) first.
// So we can prevent the iterating over the options array again and again.
/// The parsed options info used across CometChatKingfisher methods. Each property in this type corresponds a case member
/// in `CometChatKingfisherOptionsInfoItem`. When a `CometChatKingfisherOptionsInfo` sent to CometChatKingfisher related methods, it will be
/// parsed and converted to a `CometChatKingfisherParsedOptionsInfo` first, and pass through the internal methods.
 struct CometChatKingfisherParsedOptionsInfo {
    ...
    var onDataReceived: [DataReceivingSideEffect]? = nil
     ...
     init(_ info: CometChatKingfisherOptionsInfo?) {
...
extension CometChatKingfisherParsedOptionsInfo {
