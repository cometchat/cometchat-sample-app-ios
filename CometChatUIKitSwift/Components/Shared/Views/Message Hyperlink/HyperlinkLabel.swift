...
@IBDesignable public class HyperlinkLabel: UILabel {
    
    // MARK: - public properties
    public weak var delegate: HyperlinkLabelDelegate?
    
    public var enabledTypes: [HyperlinkType] = [.hashtag, .url]
    ...
    public var customColor: [HyperlinkType : UIColor] = [:]
    
    public var addUnderline: [HyperlinkType : Bool] = [:]
    
    public var customFont: [HyperlinkType : UIFont] = [:]
    
    public var customAttributes: [NSRange: [NSAttributedString.Key: Any]] = [:]
    
    public var customSelectedColor: [HyperlinkType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    public func handleCustomTap(for type: HyperlinkType, handler: @escaping (String) -> ()) {
        customTapHandlers[type] = handler
    }
    
    public func removeHandle(for type: HyperlinkType) {
        switch type {
        case .hashtag:
    ...
    // MARK: - customzation
    @discardableResult
    public func customize(_ block: (_ label: HyperlinkLabel) -> ()) -> HyperlinkLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    var defaultHyperLinkElements = [HyperlinkType: [ElementTuple]]()
    ...
    fileprivate func updateAttributesWhenSelected(_ isSelected: Bool) {
        ...
        if let preAddedAttributes = customAttributes[selectedElement.range] {
            textStorage.addAttributes(preAddedAttributes, range: selectedElement.range)
        } else {
